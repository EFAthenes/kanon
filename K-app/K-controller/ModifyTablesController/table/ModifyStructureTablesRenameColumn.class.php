...
class ModifyStructureTablesRenameColumn extends AbstractTableController
{
    ...
    {  
        ...
        $this->setTitleAndBackButton(LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN"),"fa fa-pencil-alt",KRoute::makeURL(KRoutesItems::$SHOW_TABLE_EDIT_OPTIONS,[self::$GET_PARAM_TABLENAME=>$this->getTablename()]));
        ...
        $grid->addColComponent(new TitleComponent(LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_TITLE")));
        $grid->addRowComponent();
        $grid->addColComponent(new KSelectComponent(self::COL_NAME,$this->makeListOfFieldsNames(),null,false,LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_CHOOSE_COL"),false,2,10));
        $grid->addRowComponent();      
        $grid->addColComponent(new InputStringComponent($this->col_new_name,
                self::COL_NEW_NAME,
                LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_NEW_COL"),
                LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_CONDITION"),
                true,false,2,10));
        $grid->addRowComponent();        
    ...
    private function checkPostResponse() : void
    {
        ...
        {
            $this->col_new_name=strtolower($this->col_new_name);
            if(!isAlphaNumericAndUndescore($this->col_new_name) ||!isPhpVariableNameCompliant($this->col_new_name))
            {
                $notify = new KAlertComponent(LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_LABEL_ERROR"),
                        LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_ERROR_1"), KAlertComponent::$TYPE_ERROR);
                $this->addComponent($notify);
            }                       
            else if(!in_array($this->col_value,$this->getArrayFieldsName()))
            {
                $notify = new KAlertComponent(LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_LABEL_ERROR"),
                        LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_ERROR_1"), KAlertComponent::$TYPE_ERROR);
                $this->addComponent($notify);
            }
            else if(in_array($this->col_new_name,$this->getKObject()->fieldsLabelToArray(false)))
            {
                $notify = new KAlertComponent(LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_LABEL_ERROR"),
                        LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_ERROR_1"), KAlertComponent::$TYPE_ERROR);
                $this->addComponent($notify);
            ...
            {  
                ...
                $kField->setPendingRename($this->col_new_name);
                $list= new ArrayList();
                $list->add($kField);
                if(Sql::getInstance()->renameFields($this->getTablename(),$list))
                {                
                    $notify = new KAlertComponent(LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_SUCCESS")
                            , "OK => '".$this->col_value."' => '".$this->col_new_name."' !!!", KAlertComponent::$TYPE_SUCCESS);
                    $this->addComponent($notify);
                    $this->col_value="";
                    $this->col_new_name="";
                ...
                {
                    $notify = new KAlertComponent(LanguageManager::_("SHOW_EDIT_TABLE_OPTIONS_RENAME_COLUMN_LABEL_ERROR"), Sql::getInstance()->getError(), KAlertComponent::$TYPE_ERROR);
                    $this->addComponent($notify);                       
                }
            }
        }        
