...
class KUploadFilesToDirectory extends KController
{
    public function execute(): bool
    {
        $token=SessionMemory::getInstance()->get(KUploadFiles::UPLOAD_TOKEN);
        $token_get="";
        try
        {
            ...
            switch ($_FILES['file']['error'])
            {
                ...
                case UPLOAD_ERR_INI_SIZE:
                    throw new RuntimeException('Problem with file Size.');
                case UPLOAD_ERR_FORM_SIZE:
                    throw new RuntimeException('Exceeded filesize limit.');
                default:
                    throw new RuntimeException('Unknown errors. =>'.$_FILES['file']['error']);
            }
            ...
            {
                $this->cleanDir($directory_temp_upload);
            }
    ...
    private function cleanDir(KFile $dir) : void
    {
        if($dir->exists() && $dir->isDirectory())
        {
            $listDir=$dir->listFilesToList();
            /* @var  $subDir KFile */
            foreach ($listDir as $subDir)
            {
                ...
                else if($subDir->isDirectory())
                {
                    ...
                    $datetime2 = date_create($subDir->getLastModified());
                    $interval = date_diff($datetime1, $datetime2);
                    $days = $interval->format('%d'); // H '%d'
                    if($days>1)
                    {
                        $subDir->delete();
                    }
