...
class KFieldText extends KField
{  
    function __construct()
    {
        $this->setType(static::$TEXT);
        $this->labelError="KFieldText::";
        $this->setDefault("");
    }
    ...
    function affectValue(mixed $value,bool $defaultValue=false) : bool
    {
        ...
        if(!is_string($value))
        {
            $this->setKerror($this->labelError.$this->getStringMethodError($defaultValue)."[".$this->getName()."] => Value is not Text ->".$value);
            return false;
        }
